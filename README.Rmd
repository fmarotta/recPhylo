...
recphylo_layout <- layout_recphylo(recphylo_xml, branch_length_scale = 3)
...
load_all()
l <- RecPhyloLayout$new(recphylo_xml, branch_length_scale = 3, use_branch_length = FALSE)
spEdges <- l$spLayout$edges
geneNodes <- l$recGeneLayout$nodes
geneEdges <- l$recGeneLayout$edges

p <- ggplot() +
  geom_elbow(data = spEdges, aes(x, y, xend = xend, yend = yend), lineend = "round", orientation = "y") +
  geom_text(data = geneNodes, aes(x, y, label = name)) +
  geom_elbow(data = geneEdges, aes(x, y, xend = xend, yend = yend, color = lineage, linetype = paste(event_type, leg_type, sep = "_")), lineend = "round", show.legend = F, orientation = "y") +
  scale_linetype_manual(values = c("loss_vertical" = 2, "transferBack_lateral" = 4, "bifurcationOut_lateral" = 3), na.value = 1) +
  # scale_y_reverse() +
  # coord_radial() +
  # coord_flip() +
  # theme_void() +
...
p <- ggplot() +
  ...
  geom_text(data = geneNodes, aes(x, y, label = name)) +
  geom_segment(data = geneEdges, aes(x, y, xend = xend, yend = yend, color = lineage, linetype = paste(event_type, leg_type, sep = "_")), lineend = "round", show.legend = F, orientation = "y") +
  scale_linetype_manual(values = c("loss_vertical" = 2, "transferBack_lateral" = 4, "bifurcationOut_lateral" = 3), na.value = 1) +
  # scale_y_reverse() +
  # coord_radial() +
  # coord_flip() +
...
# This is pretty cool and new!
p <- ggplot() +
  geom_curve(data = spEdges[spEdges$xend >= spEdges$x, ], aes(x, y, xend = xend, yend = yend), lineend = "round", curvature = 0.3) +
  geom_curve(data = spEdges[spEdges$xend < spEdges$x, ], aes(x, y, xend = xend, yend = yend), lineend = "round", curvature = -0.3) +
  geom_text(data = geneNodes, aes(x, y, label = name)) +
  geom_curve(data = geneEdges[geneEdges$xend >= geneEdges$x, ], aes(x, y, xend = xend, yend = yend, color = lineage, linetype = paste(event_type, leg_type, sep = "_")), lineend = "round", show.legend = F, curvature = 0.3) +
  geom_curve(data = geneEdges[geneEdges$xend < geneEdges$x, ], aes(x, y, xend = xend, yend = yend, color = lineage, linetype = paste(event_type, leg_type, sep = "_")), lineend = "round", show.legend = F, curvature = -0.3) +
  scale_linetype_manual(values = c("loss_vertical" = 2, "transferBack_lateral" = 4, "bifurcationOut_lateral" = 3), na.value = 1) +
  # scale_y_reverse() +
  # coord_radial() +
