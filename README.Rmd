...
load_all()
l <- RecPhyloLayout$new(recphylo_xml, branch_length_scale = 5, use_branch_length = FALSE)
spEdges <- l$spLayout$edges
geneNodes <- l$recGeneLayout$nodes
geneEdges <- l$recGeneLayout$edges

p <- ggplot() +
  geom_diagonal(data = spEdges, aes(x, y, xend = xend, yend = yend), lineend = "round", orientation = "y") +
